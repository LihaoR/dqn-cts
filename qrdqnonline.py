...
# -*- coding: utf-8 -*-
"""
Created on Fri Dec 21 16:31:24 2018
...
import gym
import random
from atari_wrappers_copy import wrap_deepmind
from time import sleep
import cts_density_model
...
def rescale_reward(reward):
    return np.sign(reward) * np.minimum(1., np.abs(reward))
...
class Worker():
    def __init__(self,env,name,s_size,a_size,trainer,model_path,global_episodes):
        ...
        self.env = env
        self.density = cts_density_model.CTSDensityModel()
        ...
        with sess.as_default(), sess.graph.as_default():
            while not coord.should_stop():
                ...
                while not d:
                    ...
                    s1, rew, d, _ = self.env.step(a)
                    current_frame = s1[..., -1]
                    bonus = self.density.update(current_frame) / 3.
                    #bonus = self.density.density_model.update(current_frame)
                    bonuses.append(bonus)
                    r = rescale_reward(rew + bonus)
                ...
                """
                print 'loss', l, 'Qtargetmean', np.mean(Q_target), np.mean(bonuses)
                ...
                if episode_count % 5 == 0 and episode_count != 0:
                    ...
                    print 'loss', l, 'Qtargetmean', np.mean(Q_target), np.max(Q_target)
...
s_size = 28224 # Observations are greyscale frames of 84 * 84 * 1
load_model = False
N = 20
k = 1.
model_path = './model'
